    ...
    <section class="bg-white dark:bg-gray-900">
        <div class="py-8 px-4 h-dvh text-center lg:py-16 lg:px-6">
            <div class="mx-auto mb-5 max-w-screen-sm lg:mb-10">
                ...
                <h2 class="mb-3 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">{{ $character->name }}  <small>[Alt]</small></h2>
            ...
            <div class="grid gap-8 lg:gap-16 sm:grid-cols-2 md:grid-cols-6 lg:grid-cols-6">
                @foreach ($character->main as $main)
                    <div class="text-center text-gray-500 dark:text-gray-400">
                        <img class="mx-auto mb-3 w-10 h-10 rounded-full" src="{{url('images/'.rand(1, 6).'.webp')}}" alt="{{ $main->name }}">
                        <h3 class="mb-1 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                            <a href="{{url('main/'.$main->id)}}">{{$main->name}}</a>
                        </h3>
                        <hr>     
                        <p>Main | {{$main->tribe}}</p>
                        <hr>                        
